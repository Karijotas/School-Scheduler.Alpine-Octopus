...
export function ScheduleView() {
  ...
  const createLessonOnSchedule = (props) => {
    fetch(
      `/alpine/api/v1/schedule/${params.id}/create/${lesson.id === undefined || lesson.id === "" ? props.id : lesson.id}/${props.startTime}/${props.endTime}/?givenBoolean=${online}${room.id ? `&roomId=${room.id}` : ""}${teacher.id ? `&teacherId=${teacher.id}` : ""}`, {
      method: 'PATCH'
    })
      .then(setActive(true))
      .then(applyResult);
