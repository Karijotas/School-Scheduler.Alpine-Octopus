...
export function ViewModules() {
  ...
  const fetchFilterModules = async () => {
    fetch(`http://192.168.0.129:8081/scheduler/api/v1/modules/page/name-filter/${nameText}?page=` + activePage)
  ...
  const fetchSingleModules = () => {
    fetch("http://192.168.0.129:8081/scheduler/api/v1/modules")
      .then((response) => response.json())
  ...
  const fetchModules = async () => {
    fetch(`http://192.168.0.129:8081/scheduler/api/v1/modules/page?page=` + activePage)
      .then((response) => response.json())
      .then((jsonRespones) => setModules(jsonRespones));
  };

  const removeModule = (id) => {
    fetch("http://192.168.0.129:8081/scheduler/api/v1/modules/" + id, {
      method: "DELETE",
