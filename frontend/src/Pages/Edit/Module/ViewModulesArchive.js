...
export function ViewModulesArchive() {
  ...
  const fetchSingleModules = () => {
    fetch("/alpine-octopus/api/v1/modules/archive/")
  ...
  const fetchPagedModules = async () => {
    fetch('/alpine-octopus/api/v1/modules/archive/page?page=' + activePage)
      .then((response) => response.json())
  ...
  const restoreModule = (id) => {
    fetch("/alpine-octopus/api/v1/modules/restore/" + id, {
      method: "PATCH",
