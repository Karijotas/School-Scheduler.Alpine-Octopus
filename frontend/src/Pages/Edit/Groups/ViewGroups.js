...
export function ViewGroups() {
  ...
  const fetchProgramGroups = async () => {
    fetch("/alpine/api/v1/groups/program-filter/" + programText)
      .then((response) => response.json())
      .then((jsonResponse) => setGroups(jsonResponse));
  };
  const fetchYearGroups = async () => {
    fetch("/alpine/api/v1/groups/year-filter/" + yearText)
      .then((response) => response.json())
      .then((jsonResponse) => setGroups(jsonResponse));
  };

  const fetchFilterGroups = async () => {
    fetch("/alpine/api/v1/groups/name-filter/" + nameText)
      .then((response) => response.json())
      .then((jsonResponse) => setGroups(jsonResponse));
  };

  const fetchGroups = async () => {
    fetch("/alpine/api/v1/groups/page?page=" + activePage)
      .then((response) => response.json())
      .then((jsonResponse) => setGroups(jsonResponse));
  };
  ...
  const removeGroup = (id) => {
    fetch("/alpine/api/v1/groups/delete/" + id, {
      method: "PATCH",
    })
      .then(fetchGroups)
      .then(setOpen(false));
