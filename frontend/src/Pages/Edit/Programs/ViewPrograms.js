...
export function ViewPrograms() {
  ...
  const fetchFilterPrograms = async () => {
    fetch(`/alpine-octopus/api/v1/programs/page/name-filter/${nameText}`)
  ...
  const fetchPrograms = async () => {
    fetch(`/alpine-octopus/api/v1/programs/page?page=` + activePage)
      .then((response) => response.json())
      .then((jsonRespones) => setPrograms(jsonRespones));
  };

  const removeProgram = (id) => {
    fetch("/alpine-octopus/api/v1/programs/delete/" + id, {
      method: "PATCH", 
      })    
    .then(fetchPrograms)
      .then(setOpen(false));
