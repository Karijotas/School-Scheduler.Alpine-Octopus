...
export function ViewShifts() {
  ...
  const fetchFilterShifts = async () => {
    fetch(`/alpine-octopus/api/v1/shifts/page/name-filter/${nameText}`)
  ...
  const fetchShifts = async () => {
    fetch(`/alpine-octopus/api/v1/shifts/page?page=` + activePage)
      .then((response) => response.json())
      .then((jsonRespones) => setShifts(jsonRespones));
  };

  const removeShift = (id) => {
    fetch("/alpine-octopus/api/v1/shifts/delete/" + id, {
      method: "PATCH",
    }).then(fetchShifts)
    .then(setOpen(false));
  };
