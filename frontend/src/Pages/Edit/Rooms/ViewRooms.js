...
export function ViewRooms() {
    ...
    const fetchRooms = async () => {
        fetch(`http://192.168.0.129:8081/scheduler/api/v1/rooms/page?page=` + activePage)
            .then(response => response.json())
            .then(jsonResponse => setRooms(jsonResponse));
    };

    const fetchFilterRooms = async () => {
        fetch(`http://192.168.0.129:8081/scheduler/api/v1/rooms/page/name-filter/${nameText}?page=` + activePage)
            .then((response) => response.json())
    ...
    const removeRoom = (id) => {
        fetch('http://192.168.0.129:8081/scheduler/api/v1/rooms/' + id, {
            method: 'DELETE',
    ...
    const fetchBuildingRooms = async () => {
        fetch(`http://192.168.0.129:8081/scheduler/api/v1/rooms/page/building-filter/${buildingText}?page=` + activePage)
            .then((response) => response.json())
            .then((jsonRespone) => setRooms(jsonRespone));
    };

    const fetchSingleRooms = async () => {
        fetch('http://192.168.0.129:8081/scheduler/api/v1/rooms/')
            .then(response => response.json())
